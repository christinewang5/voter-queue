...
public class PrecinctNames {
    public static List<String> precinctNames;
    ...
    /** New init method,
     * */
    public PrecinctNames(VoteService voteService) {
        List<NameModel> names = voteService.getPrecinctNameList();
        Comparator<NameModel> sortByPrec= Comparator.comparingInt(NameModel::getPrecinct);
        names.sort(sortByPrec);
        int max_l=-2147483648; //Set to minInt
        int min_l=2147483647; //Set to maxInt.
        precinctNames = new ArrayList<>();
        for (NameModel n : names) {
            precinctNames.add(n.getName());
            int curr_l = n.getPrecinct();
